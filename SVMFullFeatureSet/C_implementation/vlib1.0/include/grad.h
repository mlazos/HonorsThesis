...
/* compute gradient magniture */
 static void gradient(image<float> *src, image<float> **ret_mag, int step) {
  int width = src->width();
  int height = src->height();
  ...
  for (int y = step; y < height-step; y++) {
    for (int x = step; x < width-step; x++) {
      float dx = imRef(src, x+step, y) - imRef(src, x-step, y);
      float dy = imRef(src, x, y+step) - imRef(src, x, y-step);
